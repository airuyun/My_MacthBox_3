...
public class AlbumBitmapCacheHelper {
    ...
    private volatile static AlbumBitmapCacheHelper instance = null;//线程安全的单例模式
    private LruCache<String, Bitmap> cache;
    private ArrayList<String> currentShowString;//用来优化图片的展示效果，保存当前显示的图片path

    private AlbumBitmapCacheHelper() {

        final int memory = (int) (Runtime.getRuntime().maxMemory()/1024 / 4);//分配1/4的运行时内存给图片显示
        ...
        tpe.execute(new Runnable() {
            ...
            public void run() {
                ...
                if (width == 0 || height == 0) {
                    ...
                    options.inSampleSize = computeScale(options, ((WindowManager) (MyApp.getInstance().getSystemService(Context.WINDOW_SERVICE))).getDefaultDisplay().getWidth(), ((WindowManager) (MyApp.getInstance().getSystemService(Context.WINDOW_SERVICE))).getDefaultDisplay().getWidth());
